...
public interface BusinessCalendarRepository extends JpaRepository<BusinessCalendar, Long> {
    ...
    @Query(
            "SELECT MAX(bc.businessDate) FROM BusinessCalendar bc WHERE bc.tenantId = :tenantId AND bc.eodComplete = true")
    Optional<LocalDate> findLastCompletedEodDate(@Param("tenantId") String tenantId);

    @Query(
            "SELECT bc FROM BusinessCalendar bc WHERE bc.tenantId = :tenantId AND bc.businessDate = "
