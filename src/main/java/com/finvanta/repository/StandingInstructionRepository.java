...
public interface StandingInstructionRepository extends JpaRepository<StandingInstruction, Long> {
     ...
     */
    @Query("SELECT si FROM StandingInstruction si WHERE si.tenantId = :tenantId " + "AND si.status = 'ACTIVE' "
    ...
    List<StandingInstruction> findByTenantIdAndLoanAccountNumber(String tenantId, String loanAccountNumber);

    /** All SIs for a customer (for CIF-level SI management) */
    List<StandingInstruction> findByTenantIdAndCustomerIdOrderByStatusAscPriorityAsc(String tenantId, Long customerId);
    ...
    boolean existsActiveLoanEmiSI(
            @Param("tenantId") String tenantId, @Param("loanAccountNumber") String loanAccountNumber);
