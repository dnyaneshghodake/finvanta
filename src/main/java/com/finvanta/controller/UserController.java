...
public class UserController {
        ...
        try {
            ...
            if (password == null || password.length() < 8) {
                throw new BusinessException("WEAK_PASSWORD",
                    "Password must be at least 8 characters per RBI IT Governance");
            }
            ...
            auditService.logEvent("AppUser", saved.getId(), "USER_CREATED",
                null, username, "USER_MANAGEMENT",
    ...
    public String toggleActive(@PathVariable Long id, RedirectAttributes redirectAttributes) {
        ...
        try {
            ...
            auditService.logEvent("AppUser", user.getId(),
                newState ? "USER_ACTIVATED" : "USER_DEACTIVATED",
                String.valueOf(!newState), String.valueOf(newState), "USER_MANAGEMENT",
                "User " + (newState ? "activated" : "deactivated") + ": " + user.getUsername()
                    + " by " + SecurityUtil.getCurrentUsername());
    ...
    public String unlockUser(@PathVariable Long id, RedirectAttributes redirectAttributes) {
        ...
        try {
            ...
            user.setUpdatedBy(SecurityUtil.getCurrentUsername());
            userRepository.save(user);
            auditService.logEvent("AppUser", user.getId(), "USER_UNLOCKED",
                "LOCKED", "UNLOCKED", "USER_MANAGEMENT",
