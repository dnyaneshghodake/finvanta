...
public class DashboardController {
    ...
    public ModelAndView dashboard() {
        ...
        mav.addObject("totalCustomers", customerRepository.countByTenantIdAndActiveTrue(tenantId));
        mav.addObject(
                "pendingApplications",
        ...
        mav.addObject("pendingApprovals", workflowService.getPendingApprovals().size());
        ...
        BigDecimal grossNpaRatio = totalOutstanding.compareTo(BigDecimal.ZERO) > 0
                ? npaOutstanding.multiply(BigDecimal.valueOf(100)).divide(totalOutstanding, 2, RoundingMode.HALF_UP)
                : BigDecimal.ZERO;
        ...
        long casaAccountCount =
                depositAccountRepository.countByTenantIdAndAccountStatusNot(tenantId, DepositAccountStatus.CLOSED);
        ...
        BigDecimal casaRatio = totalDepositsAllTypes.compareTo(BigDecimal.ZERO) > 0
                ? casaDeposits.multiply(BigDecimal.valueOf(100)).divide(totalDepositsAllTypes, 2, RoundingMode.HALF_UP)
                : BigDecimal.ZERO;
