...
public class ProvisioningService {
    ...
    public void calculateAndPostProvisioning(LocalDate businessDate) {
        ...
        for (LoanAccount account : accounts) {
            try {
                ...
                log.error("Provisioning failed for {}: {}", account.getAccountNumber(), e.getMessage());
            }
            processed++;
        }
    ...
    public BigDecimal calculateRequiredProvision(LoanAccount account, LocalDate businessDate) {
        ...
        return outstanding.multiply(rate).divide(HUNDRED, 2, RoundingMode.HALF_UP);
    }
    ...
    public BigDecimal getProvisioningRate(LoanAccount account, LocalDate businessDate) {
        ...
        return switch (status) {
            ...
            case NPA_SUBSTANDARD -> isSecured ? SUB_STANDARD_SECURED_RATE : SUB_STANDARD_UNSECURED_RATE;
    ...
    private BigDecimal getDoubtfulRate(LoanAccount account, LocalDate businessDate) {
        ...
        long monthsInNpa = java.time.temporal.ChronoUnit.MONTHS.between(account.getNpaDate(), businessDate);
        if (monthsInNpa <= 12) {
            return DOUBTFUL_1YR_RATE;
    ...
    private void postProvisionChange(
            LoanAccount account, BigDecimal amount, LocalDate businessDate, String txnType, String narration) {
