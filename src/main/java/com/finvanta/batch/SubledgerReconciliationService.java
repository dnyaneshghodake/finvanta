...
public class SubledgerReconciliationService {
    ...
    public SubledgerReconciliationResult reconcile() {
        ...
        BigDecimal subledgerLoanPrincipal = loanAccountRepository.calculateTotalOutstandingPrincipal(tenantId);
        BigDecimal glLoanAssetNet = getGlNetDebitBalance(tenantId, GLConstants.LOAN_ASSET);
        checkBalance(
                "LOAN_PRINCIPAL",
                "Loan Outstanding Principal",
                GLConstants.LOAN_ASSET,
                subledgerLoanPrincipal,
                glLoanAssetNet,
                discrepancies);
        ...
        // Audit trail
        String status =
                balanced ? "SUBLEDGER_BALANCED" : "SUBLEDGER_IMBALANCED (" + discrepancies.size() + " discrepancies)";
        ...
        if (balanced) {
            ...
            for (SubledgerDiscrepancy d : discrepancies) {
                log.error(
                        "SUBLEDGER MISMATCH: check={}, subledger={}, gl={}, variance={}",
                        d.checkName(),
                        d.subledgerTotal(),
                        d.glTotal(),
                        d.variance());
    ...
    private void checkBalance(
        ...
        if (variance.abs().compareTo(new BigDecimal("0.01")) > 0) {
            discrepancies.add(
                    new SubledgerDiscrepancy(checkCode, checkName, glCode, subledgerTotal, glTotal, variance));
        }
    }
    ...
    private BigDecimal getGlNetDebitBalance(String tenantId, String glCode) {
        return glMasterRepository
                .findByTenantIdAndGlCode(tenantId, glCode)
    ...
    public record SubledgerReconciliationResult(boolean isBalanced, List<SubledgerDiscrepancy> discrepancies) {
