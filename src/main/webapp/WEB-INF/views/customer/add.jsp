        ...
        <div class="card-body">
            <form method="post" action="${pageContext.request.contextPath}/customer/add" class="fv-form">
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-4"><label class="form-label">Branch *</label><select name="branchId" class="form-select" required><c:forEach var="branch" items="${branches}"><option value="${branch.id}" ${branch.id == defaultBranchId ? 'selected' : ''}><c:out value="${branch.branchCode}" /> - <c:out value="${branch.branchName}" /></option></c:forEach></select></div>
