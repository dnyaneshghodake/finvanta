    ...
    <div class="fv-card">
        ...
        <div class="card-body">
            <table class="table fv-table">
                ...
                <c:if test="${not empty customer.photoIdType}"><tr><td class="fw-bold">Photo ID</td><td><c:choose><c:when test="${customer.photoIdType == 'PASSPORT'}">Passport</c:when><c:when test="${customer.photoIdType == 'VOTER_ID'}">Voter ID</c:when><c:when test="${customer.photoIdType == 'DRIVING_LICENSE'}">Driving License</c:when><c:when test="${customer.photoIdType == 'PAN_CARD'}">PAN Card</c:when><c:when test="${customer.photoIdType == 'AADHAAR'}">Aadhaar</c:when><c:when test="${customer.photoIdType == 'NREGA_CARD'}">NREGA Card</c:when><c:otherwise><c:out value="${customer.photoIdType}" /></c:otherwise></c:choose></td></tr></c:if>
                <c:if test="${not empty customer.addressProofType}"><tr><td class="fw-bold">Address Proof</td><td><c:choose><c:when test="${customer.addressProofType == 'PASSPORT'}">Passport</c:when><c:when test="${customer.addressProofType == 'VOTER_ID'}">Voter ID</c:when><c:when test="${customer.addressProofType == 'DRIVING_LICENSE'}">Driving License</c:when><c:when test="${customer.addressProofType == 'UTILITY_BILL'}">Utility Bill</c:when><c:when test="${customer.addressProofType == 'BANK_STATEMENT'}">Bank Statement</c:when><c:when test="${customer.addressProofType == 'AADHAAR'}">Aadhaar</c:when><c:when test="${customer.addressProofType == 'RATION_CARD'}">Ration Card</c:when><c:when test="${customer.addressProofType == 'RENT_AGREEMENT'}">Rent Agreement</c:when><c:otherwise><c:out value="${customer.addressProofType}" /></c:otherwise></c:choose></td></tr></c:if>
